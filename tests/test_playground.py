...
def test_configure_credentials():
    ...
    try:
        set_credentials(
            credential_file="../../../credentials.txt", type="deploy_model")
    except Exception as e:
        print(e)

    try:
        set_credentials(credential_file="../../credentials.txt",
                        type="deploy_model")
    except Exception as e:
        print(e)
...
def test_playground_sklearn():
    ...
    X_train, X_test, y_train, y_test, example_data, y_test_labels = ai.import_quickstart_data(
        "titanic")
    ...
    myplayground = ModelPlayground(
        input_type="tabular", task_type="classification", private=True)
    ...
    # deploy model
    myplayground.deploy_model(
        model_version=1, example_data=example_data, y_train=y_train)
...
def test_playground_keras():
    ...
    def preprocessor(image_filepath, shape=(192, 192)):
        ...
        # cv2 reads in images in order of blue green and red, we reverse the order for ML.
    ...
    # Generate filenames
    file_names = [('flower_competition_data/test_images/' +
                   str(i) + '.jpg') for i in range(1, 735)]
    ...
    myplayground = ModelPlayground(
        input_type="image", task_type="classification", private=False)
    ...
    myplayground.deploy_model(
        model_version=1, example_data="quickstart_materials/example_data", y_train=y_train)
...
def test_playground_pytorch():
    ...
    import aimodelshare as ai
    ai.download_data(
        "public.ecr.aws/y2e2a1d6/flower-competition-data-repository:latest")
    ...
    for category in categories:
        ...
        full_path = [os.path.join(flower_folder, file_name)
...
"""
        ...
        # Resize all the images...
        X = np.array(img)
        # Expand dims to add "1" to object shape [1, h, w, channels].
        X = np.expand_dims(X, axis=0)
        X = np.array(X, dtype=np.float32)  # Final shape for onnx runtime.
    ...
    preprocessed_image_data = [preprocessor(
        x) for x in fnames[0] + fnames[1] + fnames[2] + fnames[3] + fnames[4]]
    ...
    # extract correct prediction labels
    prediction_labels = [['daisy', 'dandelion', 'roses',
                          'sunflowers', 'tulips'][i] for i in prediction_column_index]

    # Create labels for y_test
    y_test_labels = [['daisy', 'dandelion', 'roses',
                      'sunflowers', 'tulips'][i] for i in y_test]

    # Create labels for y_train
    y_train_labels = [['daisy', 'dandelion', 'roses',
                       'sunflowers', 'tulips'][i] for i in y_train]
