...
class ModelPlayground:
    ...
    def __init__(
        ...
        if playground_url is None and any(
                [input_type is None, task_type is None, private is None]):
            raise ValueError(
        ...
        def codestring(self):
            if self.playground_url is None:
    ...
    def activate(
        ...
        if not onnx_timeout:
            ...
            force_onnx = False
        model_filepath = model_to_onnx_timed(
            model_filepath,
            timeout=onnx_timeout,
        ...
        track_artifacts = {
            "model_filepath": bool(model_filepath),
        ...
        if preprocessor_filepath is None:
        ...
        self.playground_url = model_to_api(
    ...
    def deploy(
        ...
        if "model_share" == cloudlocation:
            ...
            # model share pathway begins here
            from aimodelshare.cloud_pathway import cloud_deploy
            cloud_deploy(model_filepath=model_filepath,
                         model_type=self.model_type,
                         private=self.private,
                         categorical=self.categorical,
                         y_train=y_train,
    ...
    def get_apikey(self):
        ...
        if all(["username" in os.environ,
            ...
            return print(
                "'get_apikey()' unsuccessful. Please provide credentials with set_credentials().")
    ...
    def update_labels(self, y_train):
        ...
        s3["client"].upload_file(
            labels_json_filepath,
            os.environ.get("BUCKET_NAME"),
            unique_model_id + "/labels.json")
    ...
    def update_eval_data(self, eval_data):
        ...
        s3["client"].upload_file(
            eval_data_path,
            os.environ.get("BUCKET_NAME"),
            model_id + "/experiment/ytest.pkl")
        s3["client"].upload_file(
            eval_data_path,
            os.environ.get("BUCKET_NAME"),
            model_id + "/competition/ytest.pkl")
...
class Competition:
    ...
    def set_model_reproducibility_env(self, version=None):
        ...
        import_reproducibility_env_from_competition_model(
            apiurl=self.playground_url,
            version=version,
            submission_type=self.submission_type)
...
class Data:
    ...
    def share_dataset(
            self,
            data_directory="folder_file_path",
