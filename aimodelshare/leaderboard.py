...
def get_leaderboard(apiurl, verbose=3, columns=None, submission_type="competition"):
    ...
    leaderboard_json = requests.post(
...
def stylize_leaderboard(leaderboard, naming_convention="keras"):
    ...
    leaderboard = consolidate_leaderboard(
        leaderboard, naming_convention=naming_convention)
    ...
    if "model_config" in leaderboard.columns:
        ...
        leaderboard.model_config = leaderboard.model_config.map(
            lambda x: x[0:30]+'...')
    ...
    if task_type == "regression":
        ...
        for col, color in zip(percent_cols, percent_colors):
            board = board.bar(align="left", color=color,
                              subset=col, vmin=0, vmax=leaderboard[col].max())
        ...
        for col, color in zip(percent_cols, percent_colors):
            board = board.bar(align="left", color=color,
                              subset=col, vmin=0, vmax=1)

        board = board.set_properties(**percent_props, subset=percent_cols)
        board = board.format(lambda x: "{:.2f}%".format(
            x * 100), subset=percent_cols)
    # }}}
...
def consolidate_leaderboard(data, naming_convention="keras"):
    ...
    for i in data:
        ...
        mapping_lower = {i.lower(): mapping[i].lower(
        ) for i in mapping if i is not None and mapping[i] is not None}
        mapping_inverse_lower = {i.lower(): mapping_inverse[i].lower(
        ) for i in mapping_inverse if i is not None and mapping_inverse[i] is not None}
        ...
        try:
            ...
            if i in mapping_lower.keys():
                ...
                if not i == mapping_lower.get(i):
                    ...
                    matched_cols = [i+"_layers",
                                    mapping_lower.get(i)+"_layers"]
        ...
        try:
            if i in mapping_inverse_lower.keys() and naming_convention == 'keras':
                ...
                if not i == mapping_inverse_lower.get(i):
                    ...
                    matched_cols = [i+"_layers",
                                    mapping_inverse_lower.get(i)+"_layers"]
