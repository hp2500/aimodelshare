...
def _upload_preprocessor(preprocessor, client, bucket, model_id, model_version):
    ...
    try:
        ...
        client["client"].upload_file(
            dir_zip, bucket, model_id + "/preprocessor_v" + str(model_version) + ".zip")
...
def _update_leaderboard_public(
    ...
    try:
        ...
        leaderboardfilename = wget.download(
            s3_presigned_dict['get'][mastertable_path], out=temp+"/"+mastertable_path)
        import pandas as pd
        ...
        with open(temp+"/"+mastertable_path, 'rb') as f:
            ...
            if private:
                ...
                http_response = requests.post(
                    fileputlistofdicts[1]['url'], data=fileputlistofdicts[1]['fields'], files=files)

            else:

                http_response = requests.post(
                    fileputlistofdicts[0]['url'], data=fileputlistofdicts[0]['fields'], files=files)
...
def upload_model_dict(modelpath, s3_presigned_dict, bucket, model_id, model_version, placeholder=False, onnx_model=None):
    ...
    if placeholder == False:
        ...
        if meta_dict['ml_framework'] in ['keras', 'pytorch']:
            ...
            try:
                ...
                model_class = str(pyspark_model_from_string(
                    meta_dict['model_type']))
                if model_class.find("Voting") > 0:
                    default_config = ["No data available"]
                    model_configkeys = ["No data available"]
    ...
    try:
        ...
        putfilekeys = list(s3_presigned_dict['put'].keys())
        modelputfiles = [s for s in putfilekeys if str(
            'inspect_pd_'+str(model_version)+'.json') in s]
        ...
        with open(temp+"/"+'inspect_pd_'+str(model_version)+'.json', 'rb') as f:
            files = {'file': (temp+"/"+'inspect_pd_' +
                              str(model_version)+'.json', f)}
            http_response = requests.post(
                fileputlistofdicts[0]['url'], data=fileputlistofdicts[0]['fields'], files=files)
    except:
        pass
...
def upload_model_graph(modelpath, s3_presigned_dict, bucket, model_id, model_version, onnx_model=None):
    ...
    try:
        ...
        putfilekeys = list(s3_presigned_dict['put'].keys())
        modelputfiles = [s for s in putfilekeys if str(
            'model_graph_'+str(model_version)+'.json') in s]
...
def submit_model(
    ...
    if load_onnx_from_path:
        if model_filepath is not None:
            ...
            upload_model_dict(model_filepath, s3_presigned_dict,
                              bucket, model_id, model_version)

            upload_model_graph(model_filepath, s3_presigned_dict,
                               bucket, model_id, model_version)

        else:

            upload_model_dict(model_filepath, s3_presigned_dict,
                              bucket, model_id, model_version, placeholder=True)
    else:

        upload_model_dict(None, s3_presigned_dict, bucket,
                          model_id, model_version, onnx_model=onnx_model)

        upload_model_graph(None, s3_presigned_dict, bucket,
                           model_id, model_version, onnx_model=onnx_model)
    ...
    keys_to_extract = ["accuracy", "f1_score",
                       "precision", "recall", "mse", "rmse", "mae", "r2"]
    ...
    eval_metrics_subset_nonulls = {
        key: value for key, value in eval_metrics_subset.items() if isinstance(value, float)}
    eval_metrics_private_subset_nonulls = {
        key: value for key, value in eval_metrics_private_subset.items() if isinstance(value, float)}
    ...
    bodydatamodels_allstrings = {str(key): str(value)
    ...
    if str(response.status_code) == "200":
        ...
        # TODO: reponse 403 indicates that user needs to reset credentials.  Need to add a creds check to top of function.
        code_comp_result = ""
...
def update_runtime_model(apiurl, model_version=None, submission_type="competition"):
    ...
    if "model_share" == cloudlocation:
        ...
        data = json.dumps({"code": """from aimodelshare.model import update_runtime_model;"""+runtimemodstring, "zipfilename": "", "username": os.environ.get(
        ...
        prediction = requests.post("https://bhrdesksak.execute-api.us-east-1.amazonaws.com/dev/modeldata",
                                   headers=headers, data=json.dumps(bodydatamodelmetrics))
        ...
        if (model_source_key in file_list) & (preprocesor_source_key in file_list):
            response = bucket.copy(
                model_copy_source, model_id+"/"+'runtime_model.onnx')
            response = bucket.copy(
                preprocessor_copy_source, model_id+"/"+'runtime_preprocessor.zip')
