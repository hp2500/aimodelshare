...
def take_user_info_and_generate_api(model_filepath, model_type, categorical, labels, preprocessor_filepath,
    ...
    try:
        s3["client"].upload_file(exampledata_json_filepath, os.environ.get(
            "BUCKET_NAME"), unique_model_id + "/exampledata.json")
    except:
        pass
    try:
        s3["client"].upload_file(
            Filepath, os.environ.get("BUCKET_NAME"),  file_key)
        s3["client"].upload_file(Filepath, os.environ.get(
            "BUCKET_NAME"),  versionfile_key)
        ...
        s3["client"].upload_file(
            json_path, os.environ.get(
                "BUCKET_NAME"), unique_model_id + "/runtime_data.json"
        )
        os.remove(json_path)

        # upload model metadata
        upload_model_metadata(model, s3, os.environ.get(
            "BUCKET_NAME"), unique_model_id)

        # upload reproducibility env
        if reproducibility_env_filepath:
            upload_reproducibility_env(reproducibility_env_filepath, s3, os.environ.get(
                "BUCKET_NAME"), unique_model_id)
...
def send_model_data_to_dyndb_and_return_api(api_info, private, categorical, preprocessor_filepath,
    ...
    web_dashboard_url = (
        "https://www.modelshare.ai/detail/" + response_stringfinal)
...
def model_to_api(model_filepath, model_type, private, categorical, y_train, preprocessor_filepath,
    ...
    response = clone_base_image(
        user_session, repo_name, image_tag, "517169013426", base_image_api_endpoint, update)
    ...
    # verified metrics added to playground once 1. a model is submitted to a competition leaderboard and 2. playground owner updates runtime
...
def create_competition(apiurl, data_directory, y_test, eval_metric_filepath=None, email_list=[], public=False, public_private_split=0.5,
    ...
    s3["client"].upload_file(ytest_path, os.environ.get(
    ...
    if eval_metric_filepath is not None:
        ...
        if isinstance(eval_metric_filepath, list):
            ...
            for i in eval_metric_filepath:
                ...
                s3["client"].upload_file(i, os.environ.get(
                    "BUCKET_NAME"),  model_id + '/competition/metrics_' + eval_metric_name)
            ...
            s3["client"].upload_file(eval_metric_filepath, os.environ.get(
                "BUCKET_NAME"), model_id + '/competition/metrics_' + eval_metric_name)
    ...
    datauri = share_data_codebuild(
        account_number, os.environ.get("AWS_REGION_AIMS"), data_directory)
    ...
    _create_competitionuserauth_json(
        apiurl, email_list, public, datauri['ecr_uri'], submission_type="competition")
...
def create_experiment(apiurl, data_directory, y_test, eval_metric_filepath=None, email_list=[], public=False, public_private_split=0,
    ...
    if eval_metric_filepath is not None:
        ...
        if isinstance(eval_metric_filepath, list):
            ...
            for i in eval_metric_filepath:
                ...
                s3["client"].upload_file(i, os.environ.get(
                    "BUCKET_NAME"),  model_id + '/experiment/metrics_' + eval_metric_name)
    ...
    # create and upload json file with list of authorized users who can submit to this competition.
    _create_competitionuserauth_json(
        apiurl, email_list, public, datauri['ecr_uri'], submission_type="experiment")
    _create_public_private_split_json(
...
def _create_public_private_split_json(apiurl, split=0.5, submission_type='competition'):
    ...
    aws_client['client'].upload_file(
        tempdir.name+"/public_private_split.json", api_bucket, model_id +
        "/"+submission_type+"/public_private_split.json"
    )
...
def _create_competitionuserauth_json(apiurl, email_list=[], public=False, datauri=None, submission_type="competition"):
    ...
    aws_client['client'].upload_file(
        tempdir.name+"/competitionuserdata.json", api_bucket, model_id +
        "/"+submission_type+"/competitionuserdata.json"
    )
...
def update_access_list(apiurl, email_list=[], update_type="Add", submission_type="competition"):
    ...
    if update_type == "Replace_list":
        ...
        content_object = aws_client['resource'].Object(
            bucket_name=api_bucket, key=model_id + "/"+submission_type+"/competitionuserdata.json")
...
def _confirm_libraries_exist(requirements):
    ...
    for i in range(len(requirements)):
        ...
        if exists.status_code == 404:
            ...
            exists = requests.get(
                "https://pypi.org/project/" + requirements[i])
...
def _create_exampledata_json(model_type, exampledata_folder_filepath):
    ...
    if any([model_type.lower() == "tabular", model_type.lower() == "timeseries", model_type.lower() == "text"]):
        ...
        if isinstance(exampledata_folder_filepath, pd.DataFrame):
            ...
            # build json
            with open(temp_dir+'/exampledata.json', 'w', encoding='utf-8') as f:
                json.dump({"exampledata": data[:-2], "totalfiles": len(
                    files_to_convert)}, f, ensure_ascii=False, indent=4)
