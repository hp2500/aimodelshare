...
def take_user_info_and_generate_api(
        model_filepath,
        model_type,
        categorical,
        labels,
    ...
    user_client = boto3.client(
        'apigateway', aws_access_key_id=str(
            os.environ.get("AWS_ACCESS_KEY_ID_AIMS")), aws_secret_access_key=str(
            os.environ.get("AWS_SECRET_ACCESS_KEY_AIMS")), region_name=str(
    ...
    s3, iam, region = get_s3_iam_client(os.environ.get("AWS_ACCESS_KEY_ID_AIMS"), os.environ.get(
        "AWS_SECRET_ACCESS_KEY_AIMS"), os.environ.get("AWS_REGION_AIMS"))
    ...
    try:
        s3["client"].upload_file(
            Filepath, os.environ.get("BUCKET_NAME"), file_key)
        s3["client"].upload_file(
            Filepath,
            os.environ.get("BUCKET_NAME"),
            versionfile_key)
    ...
    sys.stdout.write('\r')
    sys.stdout.write(
        "[========                             ] Progress: 30% - Building lambda functions and updating permissions...")
    sys.stdout.flush()
    # }}}
...
def upload_reproducibility_env(reproducibility_env_file, s3, bucket, model_id):
    ...
    try:
        s3["client"].upload_file(
            reproducibility_env_file,
            bucket,
            model_id +
            "/runtime_reproducibility.json")
    except Exception as err:
        raise err
    # }}}
...
def send_model_data_to_dyndb_and_return_api(
        api_info,
        private,
        categorical,
        preprocessor_filepath,
        ...
        aishare_modelevaluation,
        model_type,
        aishare_tags,
        aishare_apicalls,
        exampledata_json_filepath,
        variablename_and_type_data="default",
        email_list=[]):
    """
    ...
    web_dashboard_url = (
        "https://www.modelshare.ai/detail/" +
        response_stringfinal)
...
def model_to_api(
    ...
    # 2. send_model_data_to_dyndb_and_return_api : to add new record to
    # database with user data, model and api related information
    ...
    if all([isinstance(email_list, list)]):
        ...
        decoded = jwt.decode(
            idtoken, options={
                "verify_signature": False})  # works in PyJWT < v2.0
        email = None
        email = decoded['email']
        ...
        return print(
            "email_list argument empty or incorrectly formatted. Please provide a list of emails for authorized playground users formatted as strings.")
    ...
    response = clone_base_image(
        user_session,
        repo_name,
        image_tag,
        "517169013426",
        base_image_api_endpoint,
        update)
    ...
    if input_dict is None:
        ...
        if(any([custom_libraries == 'TRUE', custom_libraries == 'true'])):
            requirements = input(
                "Enter all required Python libraries you need at prediction runtime (separated with commas):")
            # _confirm_libraries_exist(requirements)

        aishare_modelname = input("Model Name (for AI Model Share Website):")
        aishare_modeldescription = input(
    ...
    # verified metrics added to playground once 1. a model is submitted to a
    # competition leaderboard and 2. playground owner updates runtime
    aishare_modelevaluation = "unverified"
    # ...model with update_runtime_model()
    aishare_apicalls = 0

    # Force user to provide example data for tabular models {{{
    if any([model_type.lower() == "tabular",
            model_type.lower() == "timeseries"]):
    ...
    # Progress Update #1/6 {{{
    sys.stdout.write(
        "[===                                  ] Progress: 5% - Accessing Amazon Web Services, uploading resources...")
    sys.stdout.flush()
    # }}}
    ...
    sys.stdout.write('\r')
    sys.stdout.write(
        "[=================================    ] Progress: 90% - Finishing web dashboard...                           ")
    sys.stdout.flush()
    # }}}
...
def create_competition(
    ...
    """
    if all([isinstance(email_list, list)]):
        if any([len(email_list) > 0, public == "True", public == "TRUE", public]):
            import jwt
            idtoken = get_aws_token()
            decoded = jwt.decode(
                idtoken, options={
    ...
    datauri = share_data_codebuild(
        account_number,
        os.environ.get("AWS_REGION_AIMS"),
        data_directory)

    # create and upload json file with list of authorized users who can submit
    # to this competition.
    _create_competitionuserauth_json(
        apiurl,
        email_list,
        public,
        datauri['ecr_uri'],
        submission_type="competition")
    _create_public_private_split_json(
        apiurl, public_private_split, "competition")
    ...
    bodydata = {"unique_model_id": model_id,
                ...
                # getting rid of extra quotes that screw up dynamodb string
                # search on apiurls
                "apiurl": apiurl,
                "version": 0,
                "Private": "FALSE",
...
def create_experiment(
    ...
    _create_public_private_split_json(
        apiurl, public_private_split, "experiment")
...
def _create_public_private_split_json(
        apiurl, split=0.5, submission_type='competition'):
    import json
    if all(["AWS_ACCESS_KEY_ID_AIMS" in os.environ,
        ...
        return print(
            "'Set public-private split' unsuccessful. Please provide credentials with set_credentials().")
    ...
    with open(tempdir.name + '/public_private_split.json', 'w', encoding='utf-8') as f:
        json.dump({"public_private_split": str(split)},
                  f, ensure_ascii=False, indent=4)

    aws_client['client'].upload_file(
        tempdir.name +
        "/public_private_split.json",
        api_bucket,
        model_id +
        "/" +
        submission_type +
...
def _create_competitionuserauth_json(
        apiurl,
        email_list=[],
        public=False,
        datauri=None,
    ...
    with open(tempdir.name + '/competitionuserdata.json', 'w', encoding='utf-8') as f:
        json.dump({"emaillist": email_list, "public": str(public).upper(
        ), "datauri": str(datauri)}, f, ensure_ascii=False, indent=4)
...
def update_access_list(
    ...
    if all(["AWS_ACCESS_KEY_ID_AIMS" in os.environ,
        ...
        return print(
            "'Update unsuccessful. Please provide credentials with set_credentials().")
    ...
    if update_type == "Replace_list":
        ...
        import tempfile
        tempdir = tempfile.TemporaryDirectory()
        content_object = aws_client['resource'].Object(
            bucket_name=api_bucket,
            key=model_id +
            "/" +
            submission_type +
        ...
        if exists.status_code == 404:
            ...
            if exists.status_code == 404:
                error_message = (
                    ...
                    requirements[i] +
                    "' found in the Python Package Index (PyPI). \n"
...
def _create_exampledata_json(model_type, exampledata_folder_filepath):
    ...
    if any([model_type.lower() == "tabular",
            model_type.lower() == "timeseries",
            model_type.lower() == "text"]):
        # confirm data type is data frame, try to convert if not [necessary for
        # front end]
        if isinstance(exampledata_folder_filepath, pd.DataFrame):
            pass
        else:
            exampledata_folder_filepath = pd.DataFrame(
                exampledata_folder_filepath)
        ...
        with open(temp_dir + '/exampledata.json', 'w', encoding='utf-8') as f:
            json.dump({"exampledata": tabularjson, "totalfiles": 1},
                      f, ensure_ascii=False, indent=4)
        ...
        if isinstance(exampledata_folder_filepath, pd.DataFrame):
            ...
            # build json
            with open(temp_dir + '/exampledata.json', 'w', encoding='utf-8') as f:
                json.dump({"exampledata": data[:-2],
                           "totalfiles": len(files_to_convert)},
                          f,
                          ensure_ascii=False,
                          indent=4)
