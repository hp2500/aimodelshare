...
def export_postprocessor(postprocessor_fxn, directory, globs=globals()):
    ...
    try:
        ...
        zipObj.write(os.path.join(temp_dir, "postprocessor.py"),
                     "postprocessor.py")
        ...
        function_objects = list(inspect.getclosurevars(
            postprocessor_fxn).globals.keys())

        import sys
        modulenames = ["sklearn", "keras",
                       "tensorflow", "cv2", "resize", "pytorch"]
...
def upload_postprocessor(postprocessor_path, client, bucket, model_id, model_version):
    ...
    try:
        ...
        client["client"].upload_file(
            dir_zip, bucket, model_id + "/runtime_postprocessor" + ".zip")
    except Exception as e:
        print(e)
