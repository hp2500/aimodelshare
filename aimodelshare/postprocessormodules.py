...
# how to import a postprocessor from a zipfile into a tempfile then into
# the current session


def import_postprocessor(filepath):
    # postprocessor fxn should always be named "postprocessor" to work
    # properly in aimodelshare process.
    import tempfile
...
def export_postprocessor(postprocessor_fxn, directory, globs=globals()):
    ...
    try:
        ...
        function_objects = list(
            inspect.getclosurevars(postprocessor_fxn).globals.keys())
        ...
        savedpostprocessorobjectslist = list(
            map(savetopickle, function_objects_nomodules))

        # take savedpostprocessorobjectslist pkl files saved to tempdir to
        # zipfile
        import pickle
        import string
