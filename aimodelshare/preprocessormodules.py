...
# how to import a preprocessor from a zipfile into a tempfile then into
# the current session
...
def import_preprocessor(filepath):
    ...
    # preprocessor fxn should always be named "preprocessor" to work properly
    # in aimodelshare process.
    import tempfile
...
def export_preprocessor(preprocessor_fxn, directory, globs=globals()):
    ...
    try:
        ...
        function_objects = list(
            inspect.getclosurevars(preprocessor_fxn).globals.keys())
        ...
        def savetopickle(function_objects_listelement):
            import pickle
            pickle.dump(
                globals()[function_objects_listelement],
                open(
                    os.path.join(
                        temp_dir,
                        function_objects_listelement +
                        ".pkl"),
                    "wb"))
        ...
        # take savedpreprocessorobjectslist pkl & zip files saved to tempdir to
        # zipfile
        import pickle
        import string
