    ...
    if framework == 'keras':
        ...
        for layer_id, layer in enumerate(initializer):
            if(len(layer.dims) >= 2):
                ...
                try:
                    n_params = int(np.prod(layer.dims) +
                                   initializer[layer_id - 1].dims)
                except BaseException:
                    n_params = None
...
def _sklearn_to_onnx(model, initial_types=None, transfer_learning=None,
    ...
    if model_type == 'MLPClassifier' or model_type == 'MLPRegressor':
        ...
        n_params = []
        layer_dims = [model.n_features_in_] + \
            model.hidden_layer_sizes + [model.n_outputs_]
        ...
        if hasattr(model, 'coef_'):
            model_architecture['layers_n_params'] = [
...
def _keras_to_onnx(model, transfer_learning=None,
    ...
    # # Convert the model
    try:
        modelstringtest = "python -m tf2onnx.convert --saved-model  " + \
            temp_dir + " --output " + output_path + " --opset 13"
        ...
        if resultonnx == 0:
            ...
            raise Exception(
                'Model conversion to onnx unsuccessful.  Please try different model or submit predictions to leaderboard without submitting preprocessor or model files.')
    except BaseException:
        converter = tf.lite.TFLiteConverter.from_saved_model(
            temp_dir)  # path to the SavedModel directory
        ...
        modelstringtest = "python -m tf2onnx.convert --tflite " + \
            os.path.join(temp_dir, 'tempmodel.tflite') + " --output " + output_path + " --opset 13"
    ...
    if model_size > mem.available:
        ...
        warnings.warn(
            f"Model size ({model_size/1e6} MB) exceeds available memory ({mem.available/1e6} MB). Skipping extraction of model weights.")
    ...
    for i in keras_layers:
        ...
        if hasattr(
                i,
                'activation') and i.activation.__name__ in activation_list:
...
def _pytorch_to_onnx(model, model_input, transfer_learning=None,
    ...
    # get model type from model object
    metadata['model_type'] = str(model.__class__).split(
        '.')[-1].split("'")[0] + '()'
    ...
    name_list, layer_list, param_list, weight_list, activation_list = torch_metadata(
        model)
    ...
    model_architecture = {
        ...
        'layers_summary': {
            i: layer_list.count(i) for i in set(layer_list)},
        'layers_n_params': param_list,
        'layers_shapes': weight_list,
        'activations_sequence': activation_list,
        'activations_summary': {
            i: activation_list.count(i) for i in set(activation_list)},
        'loss': None,
        'optimizer': None}
...
def model_to_onnx(model, framework=None, model_input=None, initial_types=None,
    ...
    if model and framework is None:
...
def model_to_onnx_timed(
    ...
    if not (
        model_filepath is None or isinstance(
            model_filepath,
            str) or isinstance(
            model_filepath,
            onnx.ModelProto)):
        ...
        if force_onnx:
            ...
            try:
                ...
                while response not in {"1", "2"}:
                    response = input(
                        "Do you want to keep trying (1) or submit predictions only (2)? ")
...
def _get_metadata(onnx_model):
    ...
    try:
        ...
        if onnx_meta_dict['model_architecture'] is not None:
            onnx_meta_dict['model_architecture'] = ast.literal_eval(
                onnx_meta_dict['model_architecture'])

        if onnx_meta_dict['metadata_onnx'] is not None:
            onnx_meta_dict['metadata_onnx'] = ast.literal_eval(
                onnx_meta_dict['metadata_onnx'])
...
def _get_leaderboard_data(onnx_model, eval_metrics=None):
    ...
    activation_list = list(
        set(activation_list_keras + activation_list_pytorch))
    ...
    if metadata_raw['ml_framework'] in [
            'keras', 'pytorch'] or metadata_raw['model_type'] in [
            'MLPClassifier', 'MLPRegressor']:
        metadata['depth'] = metadata_raw['model_architecture']['layers_number']
        metadata['num_params'] = sum(
            metadata_raw['model_architecture']['layers_n_params'])

        for i in layer_list:
            if i in metadata_raw['model_architecture']['layers_summary']:
                metadata[i.lower(
                ) + '_layers'] = metadata_raw['model_architecture']['layers_summary'][i]
        ...
        for i in activation_list:
            if i in metadata_raw['model_architecture']['activations_summary']:
                if i.lower() + '_act' in metadata:
                    metadata[i.lower(
                    ) + '_act'] += metadata_raw['model_architecture']['activations_summary'][i]
                else:
                    metadata[i.lower(
                    ) + '_act'] = metadata_raw['model_architecture']['activations_summary'][i]
            else:
...
def _model_summary(meta_dict, from_onnx=False):
    ...
    if from_onnx:
        model_summary = pd.read_json(
            meta_dict['metadata_onnx']["model_summary"])
    else:
...
def inspect_model(
    ...
    if all(["username" in os.environ,
        ...
        return print(
            "'Inspect Model' unsuccessful. Please provide credentials with set_credentials().")
...
def color_pal_assign(val, naming_convention=None):
    ...
    if naming_convention == "keras":
        ...
        col_map = pd.read_csv(
            path / "color_mappings/color_mapping_pytorch.csv")
...
def stylize_model_comparison(comp_dict_out, naming_convention=None):
    ...
    for i in comp_dict_out.keys():
        ...
        if i == 'nn':
            ...
            df_styled = comp_dict_out['nn'].style.applymap(
                color_pal_assign, naming_convention=naming_convention)

            df_styled = df_styled.set_properties(**{'color': 'black'})

            df_styled = df_styled.set_caption('Model type: ' + 'Neural Network').set_table_styles(
                [{'selector': 'caption', 'props': [('color', 'white'), ('font-size', '18px')]}])

            df_styled = df_styled.set_properties(**{'color': 'black'})

            df_styled = df_styled.set_caption('Model type: ' + 'Neural Network').set_table_styles(
                [{'selector': 'caption', 'props': [('color', 'black'), ('font-size', '18px')]}])
            ...
            df_styled = df_styled.set_caption("No metadata available for model " + str(version)).set_table_styles(
                [{'selector': 'caption', 'props': [('color', 'black'), ('font-size', '18px')]}])
            ...
            df_styled = df_styled.set_caption('Model type: ' + i).set_table_styles(
                [{'selector': 'caption', 'props': [('color', 'black'), ('font-size', '18px')]}])
...
def compare_models(
    ...
    comp_dict_out = {
        i: pd.DataFrame(
            json.loads(
                compare_dict[i])) for i in compare_dict}
...
def instantiate_model(
    ...
    if all(["username" in os.environ,
        ...
        return print(
            "'Submit Model' unsuccessful. Please provide credentials with set_credentials().")
    ...
    try:
        ...
        raise Exception(
            f"Error: Received {resp.status_code} from AWS, Please check if Model Version is correct.")

    resp_dict = json.loads(resp.text)

    if resp_dict['model_metadata'] is None:
        print("Model for this version doesn't exist or is not submitted by the author")
        return None

    if reproduce:
        if resp_dict['reproducibility_env'] is not None:
    ...
    if ml_framework == 'keras':
        if trained == False or reproduce == True:
            ...
            model_weights = pickle.loads(
                _get_metadata(onnx_model)['model_weights'])
...
def _get_layer_names():
    ...
    activation_list = [
        i for i in activation_list if callable(
            getattr(
...
def _get_layer_names_pytorch():
    ...
    try:
        ...
        layer_list = [
            i for i in dir(
                torch.nn) if callable(
                getattr(
                    torch.nn,
                    i))]
        layer_list = [
            i for i in layer_list if i not in activation_list and 'Loss' not in i]
    except BaseException:
    ...
    for i in keras_layers:
        ...
        try:
            if isinstance(i.inbound_nodes[0].inbound_layers, list):
                layer_connect.append(
                    [x.name for x in i.inbound_nodes[0].inbound_layers])
            else:
            ...
            (layer_name,
             {
                 ...
                 '\n' +
                 str(layer_shape),
                 "URL": "https://keras.io/search.html?query=" +
                 layer_type.lower(),
                 "color": layer_color,
                 "style": "bold",
                 "Name": layer_name,
