...
def _keras_to_onnx(model, transfer_learning=None,
    ...
    try:
        ...
        modelstringtest = "python -m tf2onnx.convert --tflite " + \
            os.path.join(temp_dir, 'tempmodel.tflite') + \
            " --output "+output_path+" --opset 13"
...
def compare_models(apiurl, version_list="None",
    ...
    comp_dict_out = {i: pd.DataFrame(json.loads(
...
def instantiate_model(apiurl, version=None, trained=False, reproduce=False, submission_type="competition"):
    ...
    resp = requests.post(apiurl_eval, headers=headers,
...
def _get_layer_names():
    ...
    activation_list = [i for i in activation_list if callable(
        getattr(tf.keras.activations, i))]
    ...
    activation_list = activation_list + \
        ['Activation', 'ReLU', 'Softmax', 'LeakyReLU',
            'PReLU', 'ELU', 'ThresholdedReLU']

    layer_list = [i for i in dir(tf.keras.layers)]
    layer_list = [i for i in dir(tf.keras.layers)
                  if callable(getattr(tf.keras.layers, i))]
...
def _get_layer_names_pytorch():
    ...
    try:
        import torch
        layer_list = [i for i in dir(
            torch.nn) if callable(getattr(torch.nn, i))]
        layer_list = [
            i for i in layer_list if not i in activation_list and not 'Loss' in i]
    except:
...
def _get_sklearn_modules():
    ...
    for i in sklearn_modules:
        models_list = [j for j in dir(
            eval('sklearn.'+i)) if callable(getattr(eval('sklearn.'+i), j))]
...
def _get_pyspark_modules():
    try:
        if pyspark is None:
            raise Exception(
                "Error: Please install pyspark to enable pyspark features")
    except:
        raise Exception(
            "Error: Please install pyspark to enable pyspark features")

    pyspark_modules = ['ml', 'ml.feature',
                       'ml.classification', 'ml.clustering', 'ml.regression']

    models_modules_dict = {}

    for i in pyspark_modules:
        models_list = [j for j in dir(
            eval('pyspark.'+i)) if callable(getattr(eval('pyspark.'+i), j))]
        models_list = [j for j in models_list if re.match('^[A-Z]', j)]
...
def inspect_y_test(apiurl, submission_type):
    ...
    y_stats = requests.post(apiurl_eval, headers=headers,
                            data=json.dumps(post_dict))
