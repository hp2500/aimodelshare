...
class create_prediction_api_class():
    ...
    def __init__(self, model_filepath, unique_model_id, model_type, categorical, labels, apiid, custom_libraries, requirements, repo_name="", image_tag="", memory=None, timeout=90, pyspark_support=False):
        ...
        self.temp_dir_file_deletion_list = ['archive2.zip', 'archive3.zip', 'archive.zip',
                                            'archivetest.zip', 'archiveeval.zip', 'archiveauth.zip', 'main.py', 'ytest.pkl']
        ...
        self.file_objects_folder_path = os.path.join(
            self.temp_dir, 'file_objects')
    ...
    def create_prediction_api(self):
        ...
        if self.model_type == 'custom':
            ...
            data = pkg_resources.read_text(
                main, self.model_template_mapping[self.model_class][self.model_type])
            t = Template(data)
        ...
        if(self.model_type == 'custom'):
            ...
            data = pkg_resources.read_text(
                custom_approach, 'lambda_function.py')
        else:
        ...
        t = Template(pkg_resources.read_text(
            main, 'eval_lambda.txt').replace("$apikey", api_key))
        data = t.substitute(bucket_name=self.bucket_name,
                            unique_model_id=self.unique_model_id, task_type=self.task_type)
        ...
        self.aws_client.upload_file_to_s3(os.path.join(self.temp_dir, 'archive2.zip'), os.environ.get(
            "BUCKET_NAME"), self.unique_model_id+"/"+'archiveeval.zip')

        data2 = pkg_resources.read_text(
            main, 'authorization.txt').replace("$apikey", api_key)
        with open(os.path.join(self.temp_dir, 'main.py'), 'w') as file:
            file.write(data2)
        with ZipFile(os.path.join(self.temp_dir, 'archive3.zip'), 'a') as z:
            z.write(os.path.join(self.temp_dir, 'main.py'), 'main.py')
        self.aws_client.upload_file_to_s3(os.path.join(self.temp_dir, 'archive3.zip'), os.environ.get(
            "BUCKET_NAME"), self.unique_model_id+"/"+'archiveauth.zip')
        ###

        if self.model_type.lower() == 'custom':
            self.aws_client.upload_file_to_s3(os.path.join(self.temp_dir, 'exampledata.json'), os.environ.get(
                "BUCKET_NAME"), self.unique_model_id+"/"+"exampledata.json")
        ...
        lambdarole1 = json.loads(pkg_resources.read_text(
            json_templates, 'lambda_role_1.txt'))
        lambdapolicy1 = json.loads(pkg_resources.read_text(
            json_templates, 'lambda_policy_1.txt'))

        # delete role for CodeBuild if role with same name exists
        self.aws_client.delete_iam_role(lambdarolename)
        # creating role for CodeBuild
        self.aws_client.create_iam_role(lambdarolename, lambdarole1)
        # delete policy for CodeBuild if policy with same name exists
        self.aws_client.delete_iam_policy(lambdapolicyname)
        # creating policy for CodeBuild
        self.aws_client.create_iam_policy(lambdapolicyname, lambdapolicy1)
        ...
        if(self.custom_libraries == False):
            ...
            response6 = create_lambda_using_base_image(self.user_session, os.getenv(
                "BUCKET_NAME"), self.file_objects_folder_path, lambdafxnname, self.apiid, self.repo_name, self.image_tag, self.memory, self.timeout)
            ...
            requirements_file_path = os.path.join(
                self.file_objects_folder_path, 'requirements.txt')
            from aimodelshare.containerisation import deploy_container
            response6 = deploy_container(self.account_id, os.environ.get("AWS_REGION"), self.user_session, lambdafxnname,
                                         self.file_objects_folder_path, requirements_file_path, self.apiid, pyspark_support=self.pyspark_support)
        ...
        eval_code_source = {'S3Bucket': self.bucket_name,
                            'S3Key':  self.unique_model_id + "/" + "archiveeval.zip"}
        eval_layers = [self.eval_layer, self.auth_layer]
        create_lambda_function(lambdaevalfxnname, self.python_runtime,
                               role_arn, handler, eval_code_source, 90, 2048, eval_layers)

        auth_code_source = {'S3Bucket': self.bucket_name,
                            'S3Key':  self.unique_model_id + "/" + "archiveauth.zip"}
        auth_layers = [self.auth_layer]
        create_lambda_function(lambdaauthfxnname, self.python_runtime,
                               role_arn, handler, auth_code_source, 90, 512, auth_layers)
        ...
        arn_prefix = "arn:aws:execute-api:" + self.region + \
            ":" + self.account_id + ":" + api_id
        self.aws_client.add_invoke_resource_policy_to_lambda(
            lambdaauthfxnname, stmt_id, arn_prefix + "/*/*")
        ...
        self.aws_client.add_invoke_resource_policy_to_lambda(
            lambdaevalfxnname, 'apigateway-prod-3', arn_prefix + "/*/POST/eval")
        self.aws_client.add_invoke_resource_policy_to_lambda(
            lambdaevalfxnname, 'apigateway-test-3', arn_prefix + "/*/POST/eval")

        integration_response = json.loads(pkg_resources.read_text(
            json_templates, 'integration_response.txt'))

        lambdarole2 = json.loads(pkg_resources.read_text(
            json_templates, 'lambda_role_2.txt'))
        lambdarolename2 = 'lambda_invoke_function_assume_apigw_role_2'
        lambdapolicy2 = json.loads(pkg_resources.read_text(
            json_templates, 'lambda_policy_2.txt'))
        lambdapolicyname2 = 'invokelambda'

        # delete role for CodeBuild if role with same name exists
        self.aws_client.delete_iam_role(lambdarolename2)
        # creating role for CodeBuild
        self.aws_client.create_iam_role(lambdarolename2, lambdarole2)
        # delete policy for CodeBuild if policy with same name exists
        self.aws_client.delete_iam_policy(lambdapolicyname2)
        # creating policy for CodeBuild
        self.aws_client.create_iam_policy(lambdapolicyname2, lambdapolicy2)
        self.aws_client.attach_policy_to_role(
            lambdarolename2, lambdapolicyname2)

        uri_str = "arn:aws:apigateway:" + self.region + ":lambda:path/2015-03-31/functions/arn:aws:lambda:" + \
            self.region + ":" + self.account_id + \
            ':function:' + lambdafxnname + '/invocations'
        credentials = 'arn:aws:iam::'+self.account_id+':role/' + lambdarolename2
        self.aws_client.integration_setup(
            api_id, resource_id_lambda, uri_str, credentials, integration_response)

        uri_str_2 = "arn:aws:apigateway:" + self.region + ":lambda:path/2015-03-31/functions/arn:aws:lambda:" + \
            self.region + ":" + self.account_id + \
            ':function:' + lambdaevalfxnname + '/invocations'
        credentials_2 = 'arn:aws:iam::'+self.account_id+':role/' + lambdarolename2
        self.aws_client.integration_setup(
            api_id, resource_id_eval, uri_str_2, credentials_2, integration_response)
        ...
        auth_uri_str = "arn:aws:apigateway:" + os.environ.get("AWS_REGION") + ":lambda:path/2015-03-31/functions/arn:aws:lambda:"+os.environ.get(
            "AWS_REGION")+":"+self.account_id+":function:"+lambdaauthfxnname+"/invocations"
        ...
        response70 = self.user_session.client('lambda').add_permission(
            ...
            SourceArn='arn:aws:execute-api:'+self.region+':' +
            self.account_id+':'+api_id + '/authorizers/' + authorizerid
        )
        ...
        result = 'https://' + api_id + '.execute-api.' + \
            os.environ.get("AWS_REGION") + '.amazonaws.com/prod/m'
...
def delete_deployment(apiurl, confirmation=True):
    ...
    if "model_share" == cloudlocation:
        ...
        delplaygroundstring = "delete_deployment('" + \
            apiurl+"',"+",confirmation=False)"
        import base64
        import requests
        import json
        ...
        data = json.dumps({"code": """from aimodelshare.api import delete_deployment;"""+delplaygroundstring, "zipfilename": "", "username": os.environ.get(
            "username"), "password": os.environ.get("password"), "token": os.environ.get("JWT_AUTHORIZATION_TOKEN"), "s3keyid": "xrjpv1i7xe"})
        ...
        if os.environ.get("username").lower() in api_bucket:
            ...
            print(
                "Permission denied. Please provide credentials that allow administrator access to this api.")
            return
        ...
        client = boto3.client(
            'lambda', region_name=os.environ.get("AWS_REGION"))
        ...
        # delete api
        client = boto3.client(
            'apigateway', region_name=os.environ.get("AWS_REGION"))
        ...
        try:
            content_object = s3.Object(
                bucket_name=api_bucket, key=model_id + "/competitionuserdata.json")
