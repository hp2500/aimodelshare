...
class create_prediction_api_class():
    ...
    def __init__(
        ...
        try:
            onnx_size = math.ceil(os.path.getsize(
                model_filepath) / (1024 * 1024))
        except BaseException:
            onxx_size = 500

        self.temp_dir_file_deletion_list = [
            'archive2.zip',
            'archive3.zip',
            'archive.zip',
            'archivetest.zip',
            ...
            'main.py',
            'ytest.pkl']
        self.memory = self.memory_model_mapping[self.model_type] if self.memory is None else memory
        self.timeout = self.timeout_model_mapping[self.model_type] if self.timeout is None else timeout
    ...
    def create_prediction_api(self):
        ...
        data2 = pkg_resources.read_text(
            main, 'authorization.txt').replace(
            "$apikey", api_key)
        with open(os.path.join(self.temp_dir, 'main.py'), 'w') as file:
            file.write(data2)
        ...
        lambdarole1 = json.loads(
            pkg_resources.read_text(
                json_templates,
                'lambda_role_1.txt'))
        lambdapolicy1 = json.loads(
            pkg_resources.read_text(
                json_templates,
                'lambda_policy_1.txt'))
        ...
        self.aws_client.create_iam_role(
            lambdarolename, lambdarole1)  # creating role for CodeBuild
        # delete policy for CodeBuild if policy with same name exists
        self.aws_client.delete_iam_policy(lambdapolicyname)
        self.aws_client.create_iam_policy(
            lambdapolicyname,
            lambdapolicy1)  # creating policy for CodeBuild
        ...
        if(self.custom_libraries == False):
            ...
            response6 = create_lambda_using_base_image(
                self.user_session,
                os.getenv("BUCKET_NAME"),
                self.file_objects_folder_path,
                lambdafxnname,
                self.apiid,
                self.repo_name,
                self.image_tag,
            ...
            response6 = deploy_container(
                ...
                self.apiid,
                pyspark_support=self.pyspark_support)
        ...
        self.aws_client.create_iam_role(
            lambdarolename2, lambdarole2)  # creating role for CodeBuild
        # delete policy for CodeBuild if policy with same name exists
        self.aws_client.delete_iam_policy(lambdapolicyname2)
        self.aws_client.create_iam_policy(
            lambdapolicyname2,
            lambdapolicy2)  # creating policy for CodeBuild
        ...
        uri_str = "arn:aws:apigateway:" + self.region + ":lambda:path/2015-03-31/functions/arn:aws:lambda:" + \
            self.region + ":" + self.account_id + ':function:' + lambdafxnname + '/invocations'
        ...
        uri_str_2 = "arn:aws:apigateway:" + self.region + ":lambda:path/2015-03-31/functions/arn:aws:lambda:" + \
            self.region + ":" + self.account_id + ':function:' + lambdaevalfxnname + '/invocations'
            ...
            patchOperations=[
                {
                    "op": "replace",
                    "path": "/policy",
                    "value": '{"Version": "2012-10-17","Statement": [{"Effect": "Allow","Principal": "*","Action": "execute-api:Invoke","Resource": "arn:aws:execute-api:' +
                    os.environ.get("AWS_REGION") +
                    ':' +
                    self.account_id +
                    ':' +
...
def create_prediction_api(model_filepath, unique_model_id, model_type,
    ...
    api_class = create_prediction_api_class(
        model_filepath,
        unique_model_id,
        model_type,
        categorical,
...
def delete_deployment(apiurl, confirmation=True):
    ...
    if confirmation:
        ...
        if confirmation.lower() == "permanently delete" or confirmation.lower(
        ) == "'permanently delete'":
            pass
        else:
            return print(
                "'Delete Deployment' unsuccessful: operation cancelled by user.")
    else:
        pass
    import os
    ...
    if "model_share" == cloudlocation:
        ...
        data = json.dumps(
            {
                "code": """from aimodelshare.api import delete_deployment;""" +
                delplaygroundstring,
                "zipfilename": "",
        ...
        if all(["AWS_ACCESS_KEY_ID_AIMS" in os.environ,
            ...
            return print(
                "'Delete Deployment' unsuccessful. Please provide credentials with set_credentials().")
