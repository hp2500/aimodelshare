...
def create_docker_folder_codebuild(
        dataset_dir,
        dataset_name,
        template_folder,
        region,
...
# def share_data_local(dataset_dir, tag='latest', python_version='3.8'):
    ...
    # docker push
    # public.ecr.aws/y2e2a1d6/aimodelshare-image-classification-public


def share_data_codebuild(
        account_id,
        region,
        dataset_dir,
        dataset_tag='latest',
    ...
    create_docker_folder_codebuild(
        dataset_dir,
        dataset_name,
        template_folder,
        region,
    ...
    codebuild_trust_relationship = json.loads(
        pkg_resources.read_text(
            ...
            'codebuild_trust_relationship.txt'))
    try:
    ...
    codebuild_policies = json.loads(
        pkg_resources.read_text(
            data_sharing_templates,
            'codebuild_policies.txt'))
    try:
        create_iam_policy(
            iam,
            account_id,
            codebuild_policies_name,
            codebuild_policies)
    except BaseException:
        None
...
def share_dataset(
    ...
    response = requests.post(
        "https://jyz9nn0joe.execute-api.us-east-1.amazonaws.com/dev/modeldata",
        json=bodydata,
