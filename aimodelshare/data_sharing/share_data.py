...
def create_docker_folder_local(dataset_dir, dataset_name, python_version):
    ...
    tmp_dataset_dir = tempfile.gettempdir() + '/' + \
        '/'.join(['tmp_dataset_dir', dataset_name])
...
def create_docker_folder_codebuild(dataset_dir, dataset_name, template_folder, region, registry_uri, repository, dataset_tag, python_version):
    ...
    response = shutil.copytree(
        tmp_dataset, '/'.join([template_folder, 'tmp_dataset_dir']))
...
def share_data_codebuild(account_id, region, dataset_dir, dataset_tag='latest', python_version='3.8'):
    ...
    create_docker_folder_codebuild(dataset_dir, dataset_name, template_folder,
                                   region, registry_uri, repository, dataset_tag, python_version)

    iam = session.client('iam')

    codebuild_trust_relationship = json.loads(pkg_resources.read_text(
        data_sharing_templates, 'codebuild_trust_relationship.txt'))
    try:
        create_iam_role(iam, codebuild_role_name, codebuild_trust_relationship)
    except:
        None

    codebuild_policies = json.loads(pkg_resources.read_text(
        data_sharing_templates, 'codebuild_policies.txt'))
    try:
        create_iam_policy(
            iam, account_id, codebuild_policies_name, codebuild_policies)
    except:
        None

    try:
        attach_policy_to_role(
            iam, account_id, codebuild_role_name, codebuild_policies_name)
    except:
        None
...
def share_dataset(data_directory="folder_file_path", classification="default", private="FALSE"):
    ...
    modelplaygroundurl = input(
        "Insert AI Model Share model playground url (Optional): ")
    problemdomain = input(
        "Enter a number signifying your dataset problem domain or data type: 1 = Image 2 = Video 3 = Text 4 = Tabular 5 = Neural Style Transfer 6 = Object Detection 7 = Other \n")

    optiondict = {"1": "Image", "2": "Video", "3": "Text", "4": "Tabular",
                  "5": "Audio", "6": "Neural Style Transfer", "7": "Object Detection", "8": "Other"}
    ...
    datauri = share_data_codebuild(
        account_number, os.environ.get("AWS_REGION"), data_directory)
...
def delete_dataset(ecr_uri):
    ...
    print("The dataset accessible at '" + ecr_uri +
