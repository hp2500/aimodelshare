...
def get_auth_head_no_aws_auth(auth_url, registry, repository, type):
    resp = requests.get('{}?service={}&scope=repository:{}:pull'.format(
        auth_url, registry, repository), verify=False)
...
def get_auth_head(auth_url, registry, repository):  # to do with auth
    # no aws auth
    return get_auth_head_no_aws_auth(auth_url, registry, repository, 'application/vnd.docker.distribution.manifest.v2+json')
...
def pull_image(image_uri):
    ...
    resp = requests.get('https://{}/v2/{}/manifests/{}'.format(registry,
                        repository, tag), headers=auth_head, verify=False)

    config = resp.json()['config']['digest']
    config_resp = requests.get('https://{}/v2/{}/blobs/{}'.format(
        registry, repository, config), headers=auth_head, verify=False)
    ...
    for layer in layers:
        ...
        # done to keep from expiring
        auth_head = get_auth_head(auth_url, registry, repository)
        blobs_resp = requests.get('https://{}/v2/{}/blobs/{}'.format(
            registry, repository, layer['digest']), headers=auth_head, stream=True, verify=False)

        layer_id, layer_dir = download_layer(
            layer, layer_count, tmp_img_dir, blobs_resp)
    ...
    docker_tar = tempfile.gettempdir() + '/' + \
        '_'.join([repository.replace('/', '_'), tag]) + '.tar'
    sys.stdout.flush()
...
def extract_data_from_image(image_name, file_name, location):
    ...
    shutil.copytree(tempfile.gettempdir()+'/var/task/' +
...
def import_quickstart_data(tutorial, section="modelplayground"):
    ...
    if section == "modelplayground":
        ...
        if tutorial == "dogs":
           # instantiate model
            model = tf.keras.models.load_model(
                'dog_breed_quickstart_materials/model.h5')
        ...
        if tutorial == "covid_tweets":
            # unpack data
            X_train = pd.read_csv(
                "quickstart_covid_competition/X_train.csv").squeeze("columns")
            X_test = pd.read_csv(
                "quickstart_covid_competition/X_test.csv").squeeze("columns")
            y_test_labels = pd.read_csv(
                "quickstart_covid_competition/y_test_labels.csv").squeeze("columns")
            y_train_labels = pd.read_csv(
                "quickstart_covid_competition/y_train_labels.csv").squeeze("columns")
            # example data
        ...
        if tutorial == "imdb":
            # unpack data
            X_train = pd.read_csv(
                "imdb_quickstart_materials/X_train.csv").squeeze("columns")
            X_test = pd.read_csv(
                "imdb_quickstart_materials/X_test.csv").squeeze("columns")
            y_test_labels = pd.read_csv(
                "imdb_quickstart_materials/y_test_labels.csv").squeeze("columns")
            y_train_labels = pd.read_csv(
                "imdb_quickstart_materials/y_train_labels.csv").squeeze("columns")
            # example data
            example_data = X_train[50:55]

            # instantiate models
            lstm_model = tf.keras.models.load_model(
                'imdb_quickstart_materials/model_1.h5')
            lstm_model2 = tf.keras.models.load_model(
                'imdb_quickstart_materials/model_2.h5')
        ...
        if tutorial == "flowers":
           # instantiate model
            model = tf.keras.models.load_model(
                'quickstart_materials/flowermodel.h5')
        ...
        if tutorial == "mnist":
           # instantiate model
            model = tf.keras.models.load_model(
                'fashion_mnist_quickstart_materials/mnist_model_1.h5')
        ...
        if tutorial == "sports":
           # instantiate model
            model = tf.keras.models.load_model(
                'sports_quick_start_materials/video_1.h5')

            # unpack data
            y_train_labels = pd.read_csv(
                "sports_quick_start_materials/y_train.csv")
        ...
        if tutorial == "clickbait":
            ...
            # instantiate models
            lstm_model = tf.keras.models.load_model(
                'quickstart_clickbait_materials/lstm_model1.h5')
            lstm_model2 = tf.keras.models.load_model(
                'quickstart_clickbait_materials/lstm_model2.h5')

            # bring in data
            clickbait = pd.read_csv(
                'quickstart_clickbait_materials/clickbait_data',  sep="\n", header=None)
            clickbait['label'] = "clickbait"
            clickbait.columns = ['headline', 'label']

            not_clickbait = pd.read_csv(
                'quickstart_clickbait_materials/non_clickbait_data',  sep="\n", header=None)
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=0.2, random_state=942)
        ...
        if tutorial == "titanic":
            ...
            X = data.drop(['survived', 'sibsp', 'parch', 'ticket',
                          'name', 'cabin', 'boat', 'body', 'home.dest'], axis=1)
            ...
            training_data = training_data.drop(
                list(training_data.filter(like='Unnamed')), axis=1)
            training_data.to_csv("training_data.csv", index=False)

            test_data = X_test
            test_data = test_data.drop(
                list(test_data.filter(like='Unnamed')), axis=1)
        ...
        if tutorial == "cars":
            ...
            import pandas as pd
            data = pd.read_csv(
                "quickstart_car_sales_competition/used_car_dataset.csv")
    ...
    if section == "competition":
        ...
        if tutorial == "flowers":
            ...
            model_2 = tf.keras.models.load_model(
                'quickstart_flowers_competition/flowermodel_2.h5')
        ...
        if tutorial == "mnist":
            # Instantiate Model
            model_2 = tf.keras.models.load_model(
                'quickstart_mnist_competition/mnist_model_2.h5')
        ...
        if tutorial == "dogs":
            # Instantiate Model
            model_2 = tf.keras.models.load_model(
                'quickstart_dog_breed_competition/model_2.h5')
        ...
        if tutorial == "sports":
            model_2 = tf.keras.models.load_model(
                'quickstart_sports_competition/video_2.h5')
