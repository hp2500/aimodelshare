...
# abstraction to return list of strings of paths of all files present in a
# given directory
...
def get_repository_details(user_session, repo_name):
    ...
    try:
        repo_details = ecr_client.describe_repositories(
            repositoryNames=[repo_name])['repositories']
...
def get_image_details(user_session, repo_name, image_tag):
    ...
    ecr_client = user_session.client('ecr')
    try:
        image_details = ecr_client.describe_images(repositoryName=repo_name, imageIds=[
                                                   {'imageTag': image_tag}])['imageDetails']
...
def create_iam_role(user_session, role_name, trust_relationship):
    ...
    response = iam_client.create_role(
        ...
        AssumeRolePolicyDocument=json.dumps(
            trust_relationship)     # convert JSON to string
    )
    # give time to reflect in IAM
    time.sleep(time_delay)
...
def build_image(user_session, bucket_name, zip_file, image_name, image):
    ...
    # reading JSON of the trust relationship required to create role and
    # authorize it to use CodeBuild to build Docker image
    role_name = "codebuild_role"
    trust_relationship = json.loads(
        pkg_resources.read_text(
            iam, "codebuild_trust_relationship.txt"))
    ...
    # reading JSON of all the policies that CodeBuild requires for accessing
    # AWS services
    ...
    codebuild_project_name = 'codebuild_' + \
        image_name.replace("/", "_") + '_project'
        ...
        try:
            ...
            response = codebuild_client.create_project(
                ...
                source={
                    ...
                    # exact location of the zip in S3 bucket
                    "location": bucket_name + "/" + image_name + ".zip"
                },
                artifacts={
                ...
                environment={
                    ...
                    # type of image to use to build Docker image
                    "image": "aws/codebuild/standard:5.0",
                    # compute type to use based on the user's choice
            ...
            if(counter <= 3):
                print(
                    "CodeBuild project creation failed. Waiting for dependent resources to reflect. Retrying again in 10 seconds.")
                time.sleep(time_delay)
            else:
                ...
                delete_file_from_s3(
                    ...
                    bucket_name,
                    image_name +
                    '.zip')   # delete zip file from S3 bucket
                return

    # running through loop while build status shows termination/successful
    # completion
    counter = 0
    while(True):
        build_response = codebuild_client.batch_get_builds(
...
# create a base image containing a particular set of libraries in
# repository with specific image tag
...
def build_new_base_image(libraries, repository, image_tag, python_version):
    ...
    ##################################################
    # label = "libraries=" + ",".join(libraries)      # label of image will be
    # all string of all libraries
    ...
    # list of all Python libraries (with their versions if required) required
    # to be downloaded from PyPI into Docker image
    with open(os.path.join(temp_dir, "requirements.txt"), "a") as f:
        for lib in libraries:
            f.write('%s\n' % lib)

    # Dockerfile.txt template being read and appropriate variables being
    # assigned to generate Dockerfile
    # read template from containerization folder
    data = pkg_resources.read_text(
        containerization_templates,
        "Dockerfile.txt")
    template = Template(data)
    # AWS maintained images with speicific python versions
    newdata = template.substitute(python_version=python_version)
    with open(os.path.join(temp_dir, "Dockerfile"), "w") as file:
        file.write(newdata)

    # buildspec.txt template being read and appropriate variables being
    # assigned to generate buildspec.yml
    # read template from containerization folder
    data = pkg_resources.read_text(containerization_templates, "buildspec.txt")
    template = Template(data)
    ...
    # lambda_function.py being generated which has the handler that will be
    # called when Docker image is invoked
    data = pkg_resources.read_text(
        containerization_templates,
        "lambda_function.txt")   # read template from containerization folder
    with open(os.path.join(temp_dir, "lambda_function.py"), "w") as file:
        file.write(data)

    # getting list of strings containing paths of all files
    file_paths = get_all_file_paths_in_directory(temp_dir)

    # zipping all files in the temporary folder to be uploaded to the S3 bucket
    with zipfile.ZipFile(temp_dir + ".zip", "w") as zip:
        for file in file_paths:
            # ignore temporary file path when copying to zip file
            zip.write(file, file.replace(temp_dir, ""))
...
# create lambda function using a base image from a specific repository
# having a specific tag


def create_lambda_using_base_image(
        user_session,
        bucket_name,
        directory,
        lambda_name,
    ...
    # copying files from the local directory to tmp folder directory
    shutil.copytree(directory, temp_dir)

    temp_path_directory_file_paths = get_all_file_paths_in_directory(
        temp_dir)    # getting list of strings containing paths of all files

    # zipping all files in a temporary folder to be uploaded to the S3 bucket
    # remove temporary path from directory if present
    with zipfile.ZipFile(temp_dir + ".zip", "w") as zip:
        for file in temp_path_directory_file_paths:
            # ignore temporary path when copying to zip file
            zip.write(file, file.replace(temp_dir, ""))
    ...
    upload_file_to_s3(
        ...
        "/" +
        lambda_name +
        ".zip")        # upload zip file to S3 bucket
    ...
    trust_relationship = json.loads(
        pkg_resources.read_text(
            iam, "lambda_trust_relationship.txt"))
    ...
    code = {'ImageUri': account_id + '.dkr.ecr.' + region +
            '.amazonaws.com/' + repository + ":" + image_tag}
    package_type = "Image"
    timeout = int(timeout)
    ...
    # delete the zip file created in tmp directory
...
# check if the image exists in the specified repository with specified
# image tag
    ...
    try:
        ...
        for statement in policy_json["Statement"]:
            if(statement["Sid"] == statement_id):
                ...
                response = ecr_client.put_registry_policy(
...
# create base image having repository:image_tag identity from source
# account if image not present, update base image if update flag set to
# true


def clone_base_image(
        user_session,
        repository,
        image_tag,
        source_account_id,
    ...
    if(check_if_image_exists(user_session, repository, image_tag) and update == False):
        result = {
            ...
            ":" +
            image_tag +
            "\" already exists on this user's account."}
        ...
        bodydata = {
            ...
            "useraccountnumber": str(account_id),
            "region": region,
        ...
        result = {
            "Status": 1,
            "Success": "New base image loaded to user's account"}

        # print(result)
    else:
        result = {
            "Status": 0,
            "Success": "API endpoint not valid/not provided and image does not exist either."}
    return result
