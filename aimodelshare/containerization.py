...
def build_image(user_session, bucket_name, zip_file, image_name, image):
    ...
    trust_relationship = json.loads(pkg_resources.read_text(
    ...
    while(counter <= 3):
        try:
            ...
            if(counter <= 3):
                ...
                delete_file_from_s3(
                    user_session, bucket_name, image_name+'.zip')
                return
...
def build_new_base_image(libraries, repository, image_tag, python_version):
    ...
    data = pkg_resources.read_text(
        containerization_templates, "Dockerfile.txt")
    ...
    # read template from containerization folder
    data = pkg_resources.read_text(
        containerization_templates, "lambda_function.txt")
    ...
    build_image(user_session, bucket_name, temp_dir + ".zip",
                unique_name + "_base_image", repository+":"+image_tag)
...
def create_lambda_using_base_image(user_session, bucket_name, directory, lambda_name, api_id, repository, image_tag, memory_size, timeout):
    ...
    upload_file_to_s3(user_session, temp_dir + ".zip", bucket_name, api_id +
                      "/" + lambda_name + ".zip")        # upload zip file to S3 bucket

    # reading JSON of the trust relationship required to create role and authorize it to use CodeBuild to build Docker image
    role_name = "lambda_role_" + api_id
    trust_relationship = json.loads(
        pkg_resources.read_text(iam, "lambda_trust_relationship.txt"))
...
def clone_base_image(user_session, repository, image_tag, source_account_id, api_endpoint="", update=False):
    ...
    if(check_if_image_exists(user_session, repository, image_tag) and update == False):
        result = {"Status": 1, "Success": "Base image \"" + repository +
                  ":" + image_tag + "\" already exists on this user's account."}
        ...
        add_registry_permission(
            user_session, statement_id, source_account_id, account_id, region)
        ...
        response = requests.post(
            api_endpoint, json=bodydata, headers=headers_with_authentication)
        time.sleep(5)
        ...
        result = {
            "Status": 0, "Success": "API endpoint not valid/not provided and image does not exist either."}
    return result
